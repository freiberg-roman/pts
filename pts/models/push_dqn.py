        ...
        self.graspnet = nn.Sequential(
            OrderedDict(
                [
                    ...
                    ("grasp-relu0", nn.SiLU(inplace=True)),
                    (
                        "grasp-conv0",
                        nn.Conv2d(2048, 64, kernel_size=1, stride=1, bias=False),
                    ),
                    ("grasp-norm1", nn.BatchNorm2d(64)),
                    ("grasp-relu1", nn.SiLU(inplace=True)),
                    (
                        "grasp-conv1",
                        nn.Conv2d(64, 3, kernel_size=1, stride=1, bias=False),
                    )
    ...
    def forward(
        ...
        if is_volatile:
            ...
            for rotate_idx in range(self.num_rotations):
                ...
                if self.use_cuda:
                    rotate_color = F.grid_sample(
                        Variable(input_color_data, volatile=True).cuda(),
                        flow_grid_before,
                        mode="nearest",
                    )
                    rotate_depth = F.grid_sample(
                        Variable(input_depth_data, volatile=True).cuda(),
                        flow_grid_before,
                        mode="nearest",
                    )
                else:
                    rotate_color = F.grid_sample(
                        Variable(input_color_data, volatile=True),
                        flow_grid_before,
                        mode="nearest",
                    )
                    rotate_depth = F.grid_sample(
                        Variable(input_depth_data, volatile=True),
                        flow_grid_before,
                        mode="nearest",
                    )
                ...
                interm_push_feat = torch.cat(
                    (interm_push_color_feat, interm_push_depth_feat), dim=1
                )
                ...
                # Forward pass through branches, undo rotation on output predictions,
                # then upsample results
                output_prob.append(
                    [
                        nn.Upsample(scale_factor=16, mode="bilinear").forward(
                            F.grid_sample(
                                self.pushnet(interm_push_feat),
                                flow_grid_after,
                                mode="nearest",
                            )
                        ),
            ...
            if self.use_cuda:
                ...
                rotate_color = F.grid_sample(
                    Variable(input_color_data, requires_grad=False),
                    flow_grid_before,
                    mode="nearest",
                )
                rotate_depth = F.grid_sample(
                    Variable(input_depth_data, requires_grad=False),
                    flow_grid_before,
                    mode="nearest",
                )
...
class ReinforcementNet(nn.Module):
    ...
    def forward(
        ...
        if is_volatile:
            with torch.no_grad():
                ...
                for rotate_idx in range(self.num_rotations):
                    ...
                    if self.use_cuda:
                        with torch.no_grad():
                            rotate_color = F.grid_sample(
                                Variable(input_color_data).cuda(),
                                flow_grid_before,
                                mode="nearest",
                            )
                            rotate_depth = F.grid_sample(
                                Variable(input_depth_data).cuda(),
                                flow_grid_before,
                                mode="nearest",
                            )
                    else:
                        with torch.no_grad():
                            rotate_color = F.grid_sample(
                                Variable(input_color_data),
                                flow_grid_before,
                                mode="nearest",
                            )
                            rotate_depth = F.grid_sample(
                                Variable(input_depth_data),
                                flow_grid_before,
                                mode="nearest",
                            )

                    # Compute intermediate features
                    interm_push_color_feat = self.push_color_trunk.features(
                        rotate_color
                    )
                    interm_push_depth_feat = self.push_depth_trunk.features(
                        rotate_depth
                    )
                    interm_push_feat = torch.cat(
                        (interm_push_color_feat, interm_push_depth_feat), dim=1
            ...
            # Forward pass through branches, undo rotation on output predictions, upsample results
            # TODO remove 0 replaces grasp net
            self.output_prob.append(
                [
                    nn.Upsample(scale_factor=16, mode="bilinear").forward(
                        F.grid_sample(

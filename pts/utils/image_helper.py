...
def get_pointcloud(color_img, depth_img, camera_intrinsics):
    ...
    # np.mgrid would be better
    # pix_x, pix_y = np.mgrid[0:im_w, 0:im_h]
...
def get_difference(color_heightmap, color_space, bg_color_heightmap):
    ...
    key_color_match[key_color_match == color_space.shape[0] - 1] = (
        color_space.shape[0] + 1
    )
    bg_key_color_match[bg_key_color_match == color_space.shape[0] - 1] = (
        color_space.shape[0] + 2
    )

    return np.sum(key_color_match == bg_key_color_match).astype(float) / np.sum(
        bg_key_color_match < color_space.shape[0]
    ).astype(float)
